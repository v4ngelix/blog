...
- **`posts.json` is committed and its shape changes.** A stale `posts.json`
  paired with new `index.js` leaves `meta.tags` a string, and `"TIL".includes`
  style matching would appear to half-work. → The build must be re-run as part of
  this change and the regenerated `posts.json` committed with it. Tag handling
  reads `meta.tags` as an array via `Array.isArray`, so a stale string yields no
  tags rather than wrong ones.
...
No data migration or deployment sequencing. `posts.json` is regenerated by
`npm run build` and committed alongside the source changes. Rollback is reverting the commit; no
